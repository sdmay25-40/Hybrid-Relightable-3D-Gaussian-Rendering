...
{
    public static Gaussian3D CreateGaussian3D(Vector3 centerPos, Vector3 scale, float[] shCoefficients, float gaussOpacity)
    {
        return new Gaussian3D
        {
            pos = centerPos,
            scaleSqrd = new Vector4(scale.x * scale.x, scale.y * scale.y, scale.z * scale.z, 1),
            // spherical Harmonics coefficents
    ...
    public Gaussian3D[] ReadFile(float constScale = 1.0f)
    {
        ...
        {
            // Read position
            Vector3 pos = new Vector3(ReadNextFloat(binaryReader, fileFormat), ReadNextFloat(binaryReader, fileFormat), ReadNextFloat(binaryReader, fileFormat));
            ...
            Vector3 scale = new Vector3(ReadNextFloat(binaryReader, fileFormat), ReadNextFloat(binaryReader, fileFormat), ReadNextFloat(binaryReader, fileFormat));
            scale *= constScale;
